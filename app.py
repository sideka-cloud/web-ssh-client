...
def cleanup_inactive_sessions():
    """Periodically cleanup inactive SSH sessions"""
    while True:
        try:
            cleaned = persistent_manager.cleanup_inactive(timeout_minutes=30)
            if cleaned > 0:
                logger.info(f"Cleaned up {cleaned} inactive SSH sessions")
        except Exception as e:
...
def dashboard():
    ...
    # Calculate active connections (used within last 24 hours)
    active_count = 0
    recent_activity = []
...
def connections():
    ...
    # Order by last_used (most recent first), then by created_at
    query = query.order_by(
        SSHConnection.last_used.desc().nullslast(),
...
def test_connection():
    ...
    try:
        ...
        try:
            ...
            stdin, stdout, stderr = ssh.exec_command('echo "Connection successful"', timeout=20)
...
def execute_ssh_command():
    ...
    try:
        ...
        try:
            ...
            output = ""
            error = ""
            max_wait = 120  # Increased timeout for longer commands
    ...
    with app.app_context():
        try:
            ...
            if 'users' not in inspector.get_table_names():
                print("📦 Database belum ada, membuat database dan tabel...")
                # Buat semua tabel
                db.create_all()
                ...
                print("✅ Database sudah ada, siap digunakan")

        except Exception as e:
            print(f"❌ Error inisialisasi database: {e}")
            print("🔄 Mencoba membuat database baru...")
